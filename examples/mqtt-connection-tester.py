...
1. Provisioning (with config property)
2. Data read/write
 2.a. Sending data to devices from the platform over MQTT
 2.b. Writing data into device property over MQTT 
 2.c. Writing data into device method over MQTT
 2.d. Writing data into a child device property over MQTT
3. Connection status update
 3.a. When connected
 3.b. When disconnected
 3.c. When disabled (forcefully disconnected)
 3.d. Child device status when gateway connected 
 3.e. Child device status when gateway disconnected
 --
4. Device/App mirrors (see device-mirror-client.py)
 4.a. Connecting to device mirror and getting data
 4.b. Connecting to app mirror and getting data
"""

ConnioHost = "connio.cloud"

ConnioAPIEndpoint = "https://api.{}".format(ConnioHost)
ConnioMqttEndpoint = "mqtt.{}".format(ConnioHost)
...
def wireup(client):
    ...
    impl = MethodInstance.MethodImplementation(body="let v = value * value; Device.api.setProperty('readingSq', { value: v.toString(), time: new Date().toISOString() }).then(property => {done(null, v.toString());});", lang="javascript")    
