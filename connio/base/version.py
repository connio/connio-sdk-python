...
class Version(object):
    ...
    def exception(cls, method, uri, response, message):
        """
        Wraps an exceptional response in a `ConnioRestException`.
        """
        # noinspection PyBroadException
        try:
            error_payload = json.loads(response.content)
            return ConnioRestException(response.status_code, uri, error_payload[0]['cause'], error_payload[0]['message'], error_payload[0]['code'], method)
        except Exception:
            return ConnioRestException(response.status_code, uri, '<Unknown>', '<Unexpected error>', 'Unknown', method)
